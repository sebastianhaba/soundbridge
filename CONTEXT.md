...
- **Radio Station** — wpis w kolekcji `radio_stations` w LiteDB reprezentujący pojedynczą stację radiową. Każda stacja ma unikalną `Name`. Stacja jest prezentowana jako kontener (UPnP `storageFolder`), którego dziećmi są Streamy.

- **Stream** — wpis w kolekcji `radio_streams` w LiteDB reprezentujący pojedynczy strumień audio należący do Radio Station. Każdy stream ma `Name` (unikalna w obrębie stacji), `Url` (adres strumienia audio), `MimeType` (np. `audio/mpeg`, `audio/flac`) oraz `StationId` (referencja do stacji nadrzędnej przez ObjectId). Stream jest prezentowany jako item UPnP `audioBroadcast`, którego `<res>` wskazuje bezpośrednio na URL strumienia.
...
- **SoundBridge.Libraries.RadioOnline** — projekt z implementacją obsługi strumieni radiowych. Zawiera modele `RadioStation`, `Stream`, `IRadioOnlineStore` + `RadioOnlineStore` (LiteDB, kolekcje `radio_online_root`, `radio_stations`, `radio_streams`), `RadioOnlineResolver` (implementacja `IContentResolver`), `RadioOnlineController` (Web API CRUD). Referencjonuje `Abstractions`, `Shared` oraz NuGet `LiteDB`.
...
29. **Radio Online Library** — wirtualna biblioteka radiowa jako osobny projekt `SoundBridge.Libraries.RadioOnline` według istniejącego wzorca `Libraries.{Nazwa}` (ADR 2). Zawsze włączona (brak mechanizmu on/off na tym etapie). Domyślna nazwa roota `"Radio Online"`, edytowalna przez API. Model `RadioStation` z polem `Name` (unikalne). Model `Stream` z polami `Name` (unikalna w obrębie stacji), `Url`, `MimeType`, `StationId` (referencja ObjectId). Stacje jako kontenery z wieloma Streamami jako dziećmi (UPnP `audioBroadcast`). `<res>` wskazuje bezpośrednio na URL strumienia (nie przez `/media/`). MIME type wybierany jawnie przez użytkownika przy dodawaniu streama. Trzy kolekcje LiteDB: `radio_online_root` (pojedynczy dokument z nazwą roota, upsert), `radio_stations` (CRUD stacji, indeks unikalny na `Name`), `radio_streams` (CRUD streamów, indeks unikalny na `(StationId, Name)`). Web API: `PUT /api/radio-online` (zmiana nazwy roota), `CRUD /api/radio-online/stations/{name}`, `CRUD /api/radio-online/stations/{name}/streams/{streamName}`. ObjectID: `URL-encode({rootName}/{stationName}/{streamName})`. `ResolveToPath` zwraca `(null, null)` — brak serwowania plików przez Kestrel.
